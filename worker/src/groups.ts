...
// tool): session id, session name, group name, else materialize a new
// session. A group target fans out one inbox row per member (sender
// excluded) sharing the group's thread, then a single alarm wake covers
// every member.
