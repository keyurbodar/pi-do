...
app.get("/", (c) => Response.json({ ok: true, service: "pi-do" }));
...
app.post("/workspaces/:id/sessions/:sid/git", async (c) => {
  const workspaceId = c.req.param("id");
...
app.post("/workspaces/:id/sessions/:sid/claim", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.post("/workspaces/:id/sessions/:sid/model", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.post("/workspaces/:id/sessions/:sid/thinking", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.on(["PUT", "POST", "GET"], "/workspaces/:id/settings", async (c) => {
  const workspaceId = c.req.param("id");
...
app.post("/workspaces/:id/sessions/:sid/run", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.post("/workspaces/:id/sessions/:sid/compact", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.get("/workspaces/:id/sessions/:sid/archive", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.get("/workspaces/:id/sessions/:sid/entries", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.get("/workspaces/:id/sessions/:sid/meta", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
...
app.on(["PUT", "GET", "DELETE"], "/workspaces/:id/files", async (c) => {
  const workspaceId = c.req.param("id");
...
app.get("/workspaces/:id/bg", async (c) => {
  const workspaceId = c.req.param("id");
  if (!(await probeWorkspace(c.env, workspaceId))) return unknownWorkspace();
...
app.get("/workspaces/:id/sessions/:sid/stream", async (c) => {
  const workspaceId = c.req.param("id");
  const sessionId = c.req.param("sid");
