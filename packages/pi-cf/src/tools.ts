...
export const bashTool: AgentHarnessTool<ToolContext, any, { exit: number }> = {
  ...
  label: "Bash",
  description:
    "Run a shell command via just-bash in an isolate (no node/python). Each call gets a fresh FS, cwd pinned per call. Output capped at 1 MiB, 10s timeout. Output is captured text.",
  parameters: {
    type: "object",
