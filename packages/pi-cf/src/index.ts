...
//
// What it is not: no WS stream, no git/exec/model routes, no extensions.
// Those ride the first-party Worker until a foreign host needs them (PR19+).
    ...
    async fetch(request: Request): Promise<Response> {
      ...
      if (request.method === "POST" && url.pathname === "/run") {
        ...
        return this.enqueueSessionTurn(sid, async () => {
          ...
          try {
            const session = createAgentSession({ files: this.files, ws, shell, model, tools, apiKey });
